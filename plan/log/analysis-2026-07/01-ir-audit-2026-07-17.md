...
| Backend contract | trait + 2 proof emitters | **five-part contract FROZEN** (#3029-S1) + conformance test + **IR interchange contract v1.0** (#3030) + **an external 4th consumer (labs experiment)** | NEW | `src/ir/backend/contract.ts`, `contract-conformance.ts`, the external-backend adapter (IR-only, pinned upstream commit; moved to labs) |
...
| `src/ir/` size | 652 KB, 11 files | **33,400 lines** across `src/ir/` + `backend/` (12 files incl. the external-backend adapter) + `passes/` (8) + `analysis/` (5, new) | grew with adoption | `wc -l` |
...
v1.0 landed (#3030), and an **external-backend adapter**
(moved to labs; pinned upstream commit, IR-only by
...
- External-backend adapter depth (renderer record fidelity, drift risk vs the
