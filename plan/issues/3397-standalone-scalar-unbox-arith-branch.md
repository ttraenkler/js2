...
## Investigation (fable-dev-1, 2026-07-18) — REPRO BLOCKED on test262 harness

Branch `issue-3397-scalar-unbox` (pushed). **No code change yet** — the
producer-side fix discipline (repro-first, then A/B) could not be satisfied
because the bucket's shapes are NOT minimally reproducible outside the full
test262 harness.

### What was tried (all VALID standalone — none reproduce)

- `f64.ne found externref` (line-terminators, the 8-row plurality): the real
  `S7.3_A7_T6.js` compiles VALID both at module scope AND wrapped in
  `export function test(){…}`. The signature arises from an `eval("…result = y
  >> z…")` with ` `/` ` separators assigning a `var` that the checker
  then types externref; minimal `eval`-assign + `!== num` variants all compile
  valid (the compiler's `eval` handling differs from the harness path). Needs the
  exact wrapped+eval context.
- `i32.lt_s found f64` (Atomics `notify-one.js`): the real test references
  `$262` (harness intrinsic) → `Cannot find name '$262'` CE without the harness.
- `ref.is_null found f64/i32`, `array.len found non-array`: `num == null`,
  `f64-field == null`, `Uint8Array.toLocaleString`, `Array.prototype.
  toLocaleString.call(ta)` all compile VALID minimally.

### The ONE full-sample reproduction — and why it is NOT a #3397 shape

`Array/prototype/toLocaleString/user-provided-tolocalestring-shrink.js`
reproduces INVALID (`__closure_5`: `local.set expected (ref null 113), found
struct.get of type f64`). WAT trace: `__closure_5` is the
`Number.prototype.toLocaleString = function(){ --resizeAfter; … }` closure;
`resizeAfter` is a captured `let` (a number) materialized as a ref-CELL
`(ref null 113)`, and a `struct.get` reading the f64 OUT of the cell is fed to a
`local.set` of the CELL type. That is **captured-`let` ref-cell type drift —
the #3396 class (owned by fable-dev-5), NOT the coerceType-scalar-unbox
bridging #3397 targets.** It may be fixed by #3396 directly; if not, it belongs
in that issue, not here.

### Recommendation

#3397's real shapes need `pnpm run test:262` filtered to the 3 sample files
(with the `resizableArrayBufferUtils.js` / harness includes) to capture faithful
WATs, then trace each producer. That is a heavier, harness-backed session than a
minimal-repro loop supports. **Build DEFERRED — repro-blocked; not a coerceType
quick-win.** Re-verify the toLocale row against #3396 first (likely overlap).
