// Stylelint 17.14.1 original synchronous utility-unit slice.

import { readFileSync } from "node:fs";
import { dirname, join, relative, resolve } from "node:path";
import { fileURLToPath, pathToFileURL } from "node:url";

import { setupStylelintUpstreamSuite } from "./setup-stylelint-upstream-suite.mjs";
import {
...
const GENERATED_ROOT = resolve(HERE, "..", "..", ".stylelint-upstream-suite-generated");
const REPORT_PATH = join(HERE, "report", "stylelint-upstream-suite.json");
...
function transformStylelintTest(source, filePath, generatedPath) {
...
export async function runHarness({ quiet = false } = {}) {
  ...
  const suite = setupStylelintUpstreamSuite();
  const runs = [];

  log(`[dogfood] stylelint@${suite.pin.version} upstream ${suite.pin.tag} (${suite.pin.commit.slice(0, 12)})`);
  for (const filePath of suite.selectedPaths) {
    const file = suite.relativePath(filePath);
    const generatedPath = join(GENERATED_ROOT, file.replace(/\.mjs$/, ".ts"));
    const transformed = transformStylelintTest(readFileSync(filePath, "utf-8"), filePath, generatedPath);
  ...
  const report = summarizeUpstreamRuns({
    name: `stylelint@${suite.pin.version}`,
    pin: suite.pin,
