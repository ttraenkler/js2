...
- **Pre-push gates survive `--no-verify` (#3737)**: because `git push --no-verify` is sanctioned here (the husky integrity gate chokes on the fork/upstream divergence), it skips `.husky/pre-push` — including its lint/format lane. Two **PreToolUse** hooks therefore fire on the git-push _tool call_, which `--no-verify` cannot bypass: `pre-git-push-lint.sh` (lint + format, **auto-applies biome's safe fixes** scoped to this change-set's files and the erroring rules, then blocks so you commit the fix) and `pre-git-push-loc.sh` (#3102 LOC ratchet). Both **fail open** on any error — they can never wedge a push. Escape hatches: `LINT_AUTOFIX_SKIP=1` / `LOC_BUDGET_SKIP=1`. This matters because `quality` runs lint **first**, so one auto-fixable diagnostic also skips the ~30 gates behind it and hides whatever else is broken (exactly what happened on PR #3705).
